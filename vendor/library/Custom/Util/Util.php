 ...
 * @version CVS: Id: Util.php,v 1.0 2013-10-6 下午10:16:59 Willing Exp
 * @link http://localhost
...
// 		$url .= '222.139.198.22';
    ...
    public static function get_domain($p = true){
    	/* 协议 */
...
// 	return mb_convert_encoding($_String, 'UTF-8', 'GB2312');
    ...
    */
    ...
    static function curl_content($url) {
    ...
    static function getPointLevel($points)
    {
    	$i = 1;
    	if ($points%5000 == 0) {
    		$i += $points/5000;
    ...
    public static function sendMsg($mobiles, $content, $extParams = array())
    {
    	$config = Data::getInstance()->get('config');
    	$sms = $config['sms'];
    	$prefix = "【御订网】";
		$params = array(
			...
			'content' => urlencode($prefix.$content),
			'senddate' => date('Y-m-d H:i:s', strtotime("-1 hours")),
            'extcode' => $sms['extcode']
		);
		if (is_array($mobiles)) {
			$params['mobile'] = implode(';', $mobiles);
		} else {
			$params['mobile'] = $mobiles;
		}
    	$params = array_merge($params, $extParams);
    	$url = $sms['url'] .'?'.http_build_query($params);
    	try {
    		$config = array(
    		...
    		if ($response->getStatusCode() == 200) {
    			...
    			self::privateLogs("send msg (Mobile:{$params['mobile']});result code:".$response->getBody());
    			switch ($return) {
    				case "0":
    					$rs = array('code' => 0, 'msg' => 'success');
    					break;
    				case "-2":
    					$rs = array('code' => -2, 'msg' => '提交的号码中包含不符合格式的手机号码');
    					break;
    				case "-1":
    					$rs = array('code' => -1, 'msg' => '数据保存失败');
    					break;
    				case "1001":
    					$rs = array('code' => 1001, 'msg' => '用户名或密码错误');
    					break;
    				case "1002":
    					$rs = array('code' => 1002, 'msg' => '余额不足');
    					break;
    				case "1003":
    					$rs = array('code' => 1003, 'msg' => '参数错误，请输入完整的参数');
    					break;
    				case "1004":
    					$rs = array('code' => 1004, 'msg' => '其他错误');
    					break;
    				case "1005":
    					$rs = array('code' => 1005, 'msg' => '预约时间格式不正确');
    					break;
    				default:
    					break;
    			}
    		} else {
    			self::privateLogs("send msg (Mobile:{$params['mobile']});result code:".$response->getStatusCode());
    			$rs = array('code' => -100, 'msg' => '网络错误');
    		}
    		if ($rs['code'] == 1002) {
    			self::privateLogs($rs['msg']);
    			$rs['code'] = '抱歉，网站此功能暂时不可用！请稍后再试';
    		}
    		return $rs;
    	} catch (\Exception $e) {
    		self::privateLogs($e->getMessage());
    	}
    }
    public static function privateLogs($word) {
    	$logPath = APPLICATION_PATH."/data/files/logs/";
    	if (!file_exists($logPath)) {
    		@mkdir($logPath, 0700);
    	}
    	$fp = fopen($logPath.date('Y-m-d').'.log',"a");
    	flock($fp, LOCK_EX) ;
    	fwrite($fp, '['.date('H:i:s')."] {$word}\r\n");
    	flock($fp, LOCK_UN);
    	fclose($fp);
    }
    public static function getFormatTitle($v, $splitClass = 'split') {
    	$title = $v['title'];
    	$main_title = '';
    	$preImg = '';
    	$appendImg = '';
    	if (!empty($v['color'])) {
    		$title = "<font color='{$v['color']}'>" . $title . "</font>";
    	}
    	if ($v['main_title']) {
    		$main_title = "<font color='{$v['main_color']}'>" . $v['main_title'] . "</font>";
    	}
    	if ($v['show_icon'] && !empty($v['icon'])) {
    		$preImg = "<ins><img src='" . __IMG_URL . "{$v['icon']}' /></ins>";
    	}
    	if ($v['show_icon'] && !empty($v['post_icon'])) {
    		$appendImg = "<img src='" . __IMG_URL . "{$v['post_icon']}' />";
    	}
    	if ($main_title) {
    		$html = "<a href='{$v['main_url']}' class=\"hao-link\" title='{$v['main_title']}'"
    				." target='{$v['target']}'>"
    				. $preImg . $main_title . "</a>";
    		$html .= "<span class=\"{$splitClass}\">•</span>";
    		$html .= "<a href='{$v['url']}' class=\"hao-link\" title='{$v['title']}'"
    				." target='{$v['target']}'>"
    				. $title . $appendImg . "</a>";
    		$title = $html;
    	} else {
    		$title = "<a href='{$v['url']}' class=\"hao-link\" title='{$v['title']}'"
    				." target='{$v['target']}'>"
    				. $preImg . $title . $appendImg . "</a>";
    	}
    	$v['formatTitle'] = $title;
    	return $v;
    }
    ...
    public static function resizeImage($imgFile, $maxwidth, $maxheight, $name){
    		if($imgFile['type'] == "image/pjpeg"
    				||$imgFile['type'] == "image/jpg"
    				||$imgFile['type'] == "image/jpeg"){
    			$im = imagecreatefromjpeg($imgFile['tmp_name']);
    		}elseif($imgFile['type'] == "image/x-png"){
    			$im = imagecreatefrompng($imgFile['tmp_name']);
    		}elseif($imgFile['type'] == "image/gif"){
		 ...
		 if(($width > $maxwidth) || ($height > $maxheight)){
			  ...
			  $flg = ImageJpeg ($newim, $name . ".jpg");
			  ImageDestroy ($newim);
		 }else{
		  	$flg = ImageJpeg($im, $name . ".jpg");
		 }
