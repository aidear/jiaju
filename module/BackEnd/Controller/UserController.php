...
use BackEnd\Model\User\RoleTable;
...
use BackEnd\Model\User\UserTable;
...
class UserController extends AbstractController
{
    ...
    function saveAction(){
        ...
        if($requery->isPost()){
            ...
            $user->LastUpdate = $user->AddTime = Util::getDateTime();
            ...
            $user->Status = 1;
            $table = $this->_getUserTable();
            if ($table->checkUserIsExist($params->UserName, $params->ID)) {
            	$this->_message('用户名'.$params->UserName.'已存在！请更换其他用户名', 'error');
            	return $this->redirect()->toRoute('backend' , array('controller' => 'user' , 'action' => 'save'));
            } else {
            	$table->save($user);
